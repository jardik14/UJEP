...
# Model
model <- polr(partic ~ hincome + children + region, data=women.work, Hess=TRUE)
...
(mod.p1 <- glm(TICKS ~ HEIGHT + YEAR, family = "poisson", data = grouseticks))
summary(mod.p1)
Anova(mod.p1)


(mod.p2 <- glm(TICKS ~ HEIGHT + YEAR, family = "quasipoisson", data = grouseticks))
summary(mod.p2)
Anova(mod.p2)
...
(mod.nb <- glm.nb(TICKS ~ HEIGHT + YEAR, data=grouseticks))
summary(mod.nb)
qqnorm(residuals(mod.nb)); qqline(residuals(mod.nb), col = 2)
Anova(mod.nb)

grouseticks$YEAR <- as.factor(grouseticks$YEAR)
# graficke znazorneni (YEAR 95-97)
pred <- predict(mod.p1, type = "response")
vyst <- data.frame(grouseticks$HEIGHT, grouseticks$YEAR, pred)
vyst <- vyst[order(vyst[,1]),]
plot(TICKS ~ HEIGHT, col = YEAR, pch = 19, data = grouseticks,
     main = "Zavislost poctu skvrn na nadmorske vysce (poisson)")
lines(vyst[vyst[,2] == 95, 1], vyst[vyst[,2] == 95,3], col = 1)
lines(vyst[vyst[,2] == 96, 1], vyst[vyst[,2] == 96,3], col = 2)
lines(vyst[vyst[,2] == 97, 1], vyst[vyst[,2] == 97,3], col = 3)
legend("topright", legend = levels(grouseticks$YEAR), lty = 1, col = 1:3)
  # graficke znazorneni zavislosti poctu skvrn na nadmorske vysce
  #   pro jednotlivy rok

# grf pro mode quasipoisson
pred <- predict(mod.p2, type = "response")
vyst <- data.frame(grouseticks$HEIGHT, grouseticks$YEAR, pred)
vyst <- vyst[order(vyst[,1]),]
plot(TICKS ~ HEIGHT, col = YEAR, pch = 19, data = grouseticks,
     main = "Zavislost poctu skvrn na nadmorske vysce (quasipoisson)")
lines(vyst[vyst[,2] == 95, 1], vyst[vyst[,2] == 95,3], col = 1)
lines(vyst[vyst[,2] == 96, 1], vyst[vyst[,2] == 96,3], col = 2)
lines(vyst[vyst[,2] == 97, 1], vyst[vyst[,2] == 97,3], col = 3)
legend("topright", legend = levels(grouseticks$YEAR), lty = 1, col = 1:3)
...
# grf pro mode neg binom
pred <- predict(mod.nb, type = "response")
vyst <- data.frame(grouseticks$HEIGHT, grouseticks$YEAR, pred)
vyst <- vyst[order(vyst[,1]),]
plot(TICKS ~ HEIGHT, col = YEAR, pch = 19, data = grouseticks,
     main = "Zavislost poctu skvrn na nadmorske vysce (neg binom)")
lines(vyst[vyst[,2] == 95, 1], vyst[vyst[,2] == 95,3], col = 1)
lines(vyst[vyst[,2] == 96, 1], vyst[vyst[,2] == 96,3], col = 2)
lines(vyst[vyst[,2] == 97, 1], vyst[vyst[,2] == 97,3], col = 3)
legend("topright", legend = levels(grouseticks$YEAR), lty = 1, col = 1:3)
