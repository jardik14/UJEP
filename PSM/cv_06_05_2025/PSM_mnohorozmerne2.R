...
table(biopsy$class[train])

z <- lda(class ~ ., biopsy, prior = c(2,1)/3, subset = train)
...
hc.crabs <- hclust(dist(crabs.sc), "ave")

plot(hc.crabs, hang = -1)

seg.crabs <- cutree(hc.crabs, k = 4)

rect.hclust(hc.crabs, k=4, border="red")

# jak souhlasi deleni skupin s pohlavim
table(crabs$sex, seg.crabs)

# jak souhlais s druhem
table(crabs$sp, seg.crabs)
...
table(fgl$type[train])

z <- lda(type ~ ., fgl, prior = c(8,9,3,1,1,2)/24, subset = train)

table(fgl[-train,"type"], predict(z, fgl[-train, ])$class)
  # klasifikacni tabulka, jak dobre se trefim: v radcich skutecne hodnoty, ve sloupcich predikce

# Shluková analýza

# škálování
fgl.sc <- scale(fgl[,1:9])
  # spocitame standardizovane promenne

hc.fgl <- hclust(dist(fgl.sc[,1:9]), "ward.D2")

plot(hc.fgl, hang = -1)

seg.fgl <- cutree(hc.fgl, k = 6)

rect.hclust(hc.fgl, k=6, border="red")
# jak souhlasi deleni skupin s druhem

table(fgl$type, seg.fgl)
