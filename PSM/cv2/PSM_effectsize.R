...
# potřebujeme 163 (v každé skupině!)
...
pwr.t.test(d=5/10, sig.level = 0.05, power = 0.9, type = "two.sample")
...
load("D:/Programko/UJEP/PSM/cv2/Stulong.RData")
...
par(mfrow = c(1,2))
tapply(cis, kat, PlotQQ)
par(mfrow = c(1,1))

boxplot(cis ~ kat)

tapply(cis, kat, shapiro.test)
...
cohens_d(cis ~ kat)
# Cohenovo d
interpret_cohens_d(cohens_d(cis ~ kat))

hedges_g(cis ~ kat)
# Hedgesovo g
interpret_hedges_g(hedges_g(cis ~ kat))

glass_delta(cis ~ kat)
# Glassovo delta
interpret_glass_delta(glass_delta(cis ~ kat))

eta_squared(aov(cis ~ kat))
# Fisherovo eta
(A <- anova(aov(cis ~ kat)))
A[,2]
A[1,2]/(sum(A[,2]))
...
omega_squared(aov(cis ~ kat))
# Haysova omega
(A[1,2] - A[2,3])/(sum(A[,2]) + A[2,3])

epsilon_squared(aov(cis ~ kat))
...
plot(tlak ~ vaha, pch=19, main="Souvislost systolickeho tlaku a vahy")

cor(tlak, vaha)
cor.test(tlak, vaha)
interpret_r(cor(tlak, vaha))

summary(lm(tlak ~ vaha))$r.squared
interpret_r2(summary(lm(tlak ~ vaha))$r.squared)
...
boxplot(cukr ~ skupina)

res <- residuals(lm(cukr ~ skupina))
PlotQQ(res, pch = 19)
# nemám normální rozdělení (musím použít testy nevyžadující normalitu)


kruskal.test(cukr ~ skupina)

DunnTest(cukr ~ skupina)

# Fisherovo eta
eta_squared(aov(cukr ~ skupina))
interpret_eta_squared(0.03, rules = "cohen1992")

# Haysova omega
omega_squared(aov(cukr ~ skupina))
interpret_omega_squared(0.02, rules = "cohen1992")

epsilon_squared(aov(cukr ~ skupina))
interpret_epsilon_squared(0.02, rules = "cohen1992")
...
tlak <- Stulong$syst1
kurak <- Stulong$KOURrisk

boxplot(tlak ~ kurak)

res <- residuals(lm(tlak ~ kurak))
PlotQQ(res, pch = 19)

kruskal.test(tlak ~ kurak)
...
res <- residuals(lm(vyska ~ vino))
PlotQQ(res, pch = 19)

anova(aov(vyska ~ vino))

# Fisherovo eta
eta_squared(aov(vyska ~ vino))
interpret_eta_squared(0.03, rules = "cohen1992")

# Haysova omega
omega_squared(aov(vyska ~ vino))
interpret_omega_squared(0.02, rules = "cohen1992")

epsilon_squared(aov(vyska ~ vino))
interpret_epsilon_squared(0.02, rules = "cohen1992")
